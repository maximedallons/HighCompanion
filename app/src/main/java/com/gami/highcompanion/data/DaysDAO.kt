...
interface DaysDAO {
    ...
    @Query("SELECT SUM(smoked_joints) FROM days_table WHERE sql_date >= DATE('now', 'weekday 0', '-7 days')  AND sql_date != DATE('now')  AND sql_date != DATE('now','-1 day')")
    fun getWeeklySmokedJoints(): LiveData<Int>

    @Query("SELECT SUM(smoked_joints) FROM days_table where strftime('%W',sql_date) != strftime('%W',date('now')) AND strftime('%Y',sql_date) = strftime('%Y',date('now')) AND  strftime('%m',sql_date) = strftime('%m',date('now')) AND sql_date != DATE('now', 'weekday 0', '-7 days')  AND sql_date != DATE('now')  AND sql_date != DATE('now','-1 day')")
